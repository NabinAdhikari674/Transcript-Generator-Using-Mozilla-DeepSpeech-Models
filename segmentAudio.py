...
def silenceRemoval(buffer_dir,input_file, smoothing_window = 1.0, weight = 0.2):
    ...
    [fs, x] = read_audio_file(input_file) #returns sampleRate,Signal
    segmentLimits = silence_removal(x, fs, 0.05, 0.05, smoothing_window, weight)
    buffer = os.path.join(os.path.dirname(input_file),buffer_dir,os.path.basename(input_file))
    for i, s in enumerate(segmentLimits):
        strOut = "{0:s}_{1:.3f}-{2:.3f}.wav".format(buffer[0:-4], s[0], s[1])
        try :
